...
<style>body{{font-family:Arial,sans-serif;margin:24px}} pre{{background:#111;color:#0f0;padding:12px;height:60vh;overflow:auto}}</style></head>
<body><a href='/'>← 返回首页</a>
<h1>任务 {job.id}</h1>
...
async function refresh(){{
 ...
 if(data.status==='running'||data.status==='queued') setTimeout(refresh,1200);
}}
refresh();
</script></body></html>"""
